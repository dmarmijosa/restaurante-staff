          ...
          <div class="flex h-[640px] flex-col overflow-hidden rounded-2xl bg-marfil">
            ...
            <header class="flex flex-none items-center gap-3 border-b border-borde px-[22px] py-3.5">
              ...
              <div>
                ...
                <div class="text-[11px] text-tinta-media">{{ 'cashier.shift_label' | translate }}</div>
              </div>
              ...
              <div class="rounded-full bg-oliva-bg px-3 py-1 text-[11.5px] font-bold text-oliva-texto">
                {{ 'cashier.charged_total' | translate: { total: (chargedTotal() | money) } }}
              </div>
            </header>
            ...
            <div class="grid min-h-0 flex-1 grid-cols-[1fr_300px]">
              ...
              <section class="overflow-y-auto px-[22px] py-[18px]">
                <div class="mb-3 text-xs font-bold tracking-[.05em] text-tinta-media">{{ 'cashier.orders_title' | translate }}</div>
                  ...
                  @for (order of store.ordersToCharge(); track order.id) {
                    <article class="rounded-xl border border-borde bg-papel px-4 py-3.5">
                      ...
                      @if (store.activePaymentMethods().length) {
                        <div class="mb-1 text-[10.5px] font-semibold text-tinta-media">{{ 'cashier.charge_with' | translate }}</div>
                        ...
                        {{ 'cashier.no_payment_methods' | translate }}
                        </div>
                      }
                    </article>
                  }
              ...
              <aside class="overflow-y-auto border-l border-borde p-[18px]">
                <div class="mb-3 text-xs font-bold tracking-[.05em] text-tinta-media">{{ 'cashier.recent_title' | translate }}</div>
                @if (charged().length === 0) {
                  <div class="text-xs text-tinta-media">{{ 'cashier.no_recent' | translate }}</div>
                }
                <div class="flex flex-col gap-2">
                  @for (order of charged(); track order.id) {
                    <div class="rounded-[10px] border border-borde bg-papel px-3 py-2.5">
                      <div class="flex items-center gap-2">
                        <span class="text-[12.5px] font-bold">{{ 'admin.orders.mesa' | translate }} {{ order.tableNumber }}</span>
                        <span class="flex-1"></span>
