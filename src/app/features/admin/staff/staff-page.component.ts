...
const ROLES: StaffRole[] = ['mesero', 'cocina', 'admin'];
      ...
      <div class="mb-[18px] flex items-end gap-4">
        <div class="flex-1">
          ...
          <p class="mt-1 mb-0 text-[13px] text-tinta-media">
            Altas, bajas, vacaciones y rotación de turnos del equipo de sala y cocina.
          </p>
        </div>
        <button
          type="button"
          ...
          <div class="flex items-center gap-3.5 border-b border-panal px-[18px] py-3.5" data-testid="staff-row">
            ...
            >
              @for (role of roles; track role) {
                <option [value]="role">{{ role === 'admin' ? 'Admin' : role === 'mesero' ? 'Mesero' : 'Cocina' }}</option>
              }
            </select>
...
export class StaffPageComponent {
  ...
  protected async save(): Promise<void> {
    ...
    if (!name) {
      this.toast.show('Escribe el nombre del mesero');
      return;
    }
    await this.store.addWaiter(name, this.draftShift());
    this.draftName = '';
